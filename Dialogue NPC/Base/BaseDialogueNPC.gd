...
export(String) var quest_name = "Quest TesterNPC"
export(int) var required_amount = 10
export(String) var item_name = "bundle_filled"
export(String) var description = "Przynies 10 bundle_filled dla TesterNPC"
...
func q1_pending():
	get_tree().paused = true
	var dialog = Dialogic.start("/Testing_levels/Quest/TesterNPC/q1-T-pending")
	dialog.pause_mode = Node.PAUSE_MODE_PROCESS
	dialog.connect('timeline_end', self, 'unpause')
	add_child(dialog)
